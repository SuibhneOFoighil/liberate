...
from utils import is_null, extract_reference_numbers, NULL_ID, get_embedding
...
class PineconeKnowledgeBase:
    ...
    def query(self, prompt, K) -> tuple:
        ...
        res = self.index.query(vector=xq, top_k=K, include_metadata=True, where={'politician': self.politician})
...
class Profile:
    def __init__(self, index, shortcode):
        ...
        self.avatar = self.get_avatar(shortcode)
        self.intro = self.get_intro(shortcode)
    ...
    def get_avatar(self, shortcode):
        return POLITICIANS[self.name]["avatar"]
    
    def get_intro(self, shortcode):
        return POLITICIANS[self.name]["intro"]
